...
namespace SDT
{
    ...
    static constexpr const char* CONF_ENABLE = "EnableOSD";
    static constexpr const char* CONF_FONT = "Font";
    static constexpr const char* CONF_FONTFILE = "FontFile";
    static constexpr const char* CONF_FONTCOLOR = "Color";
    static constexpr const char* CONF_FONTOUTLINECOLOR = "OutlineColor";
    static constexpr const char* CONF_FONTOUTLINEOFFSET = "OutlineOffset";
    static constexpr const char* CONF_FPS = "FPS";
    static constexpr const char* CONF_BAREFPS = "Bare_FPS";
    static constexpr const char* CONF_FRAMETIME = "FrameTime";
    static constexpr const char* CONF_BAREFRAMETIME = "Bare_FrameTime";
    static constexpr const char* CONF_COUNTER = "Counter";
    static constexpr const char* CONF_VRAM = "VRAM";
    static constexpr const char* CONF_ALL = "All";

    static constexpr const char* CONF_LOADINGTIME = "LoadingTime";
    static constexpr const char* CONF_BARELOADTIME = "Bare_LoadingTime";

   static constexpr const char* CONF_TOGGLEKEY = "ToggleKey";
    static constexpr const char* CONF_ALIGN = "Align";
    static constexpr const char* CONF_OFFSET = "Offset";
    static constexpr const char* CONF_SCALE = "Scale";
    static constexpr const char* CONF_AUTOSCALE = "AutoScale";
    static constexpr const char* CONF_INTERVAL = "UpdateInterval";
    static constexpr const char* CONF_COMBOKEY = "ComboKey";
    static constexpr const char* CONF_SCALETOWINDOW = "ScaleToWindow";
    static constexpr const char* CONF_LOADTIMEDELAY = "LoadingTimeDelay";
    ...
    void DOSD::LoadConfig() {
        m_conf.enabled = GetConfigValueBool(SECTION_OSD, CONF_ENABLE, false);
        m_conf.fps = GetConfigValueBool(SECTION_OSD, CONF_FPS, true);
        m_conf.barefps = GetConfigValueBool(SECTION_OSD, CONF_BAREFPS, false);
        m_conf.frametime = GetConfigValueBool(SECTION_OSD, CONF_FRAMETIME, false);
        m_conf.bareframetime = GetConfigValueBool(SECTION_OSD, CONF_BAREFRAMETIME, false);
        m_conf.counter = GetConfigValueBool(SECTION_OSD, CONF_COUNTER, false);
        m_conf.vram = GetConfigValueBool(SECTION_OSD, CONF_VRAM, true);
        m_conf.all = GetConfigValueBool(SECTION_OSD, CONF_ALL, false);
        m_conf.loading_time = GetConfigValueBool(SECTION_OSD, CONF_LOADINGTIME, false);
        m_conf.bare_loading_time = GetConfigValueBool(SECTION_OSD, CONF_BARELOADTIME, false);
        if (m_conf.bare_loading_time) {
            m_conf.loading_time = false;
        }
        m_conf.delay = GetConfigValueDouble(SECTION_OSD, CONF_LOADTIMEDELAY, 5.0f);
        m_conf.interval = std::clamp(GetConfigValueFloat(SECTION_OSD, CONF_INTERVAL, 1.0f), 0.01f, 900.0f);
        m_conf.combo_key = GetConfigValue<UInt32>(SECTION_OSD, CONF_COMBOKEY, 160);
        m_conf.key = GetConfigValue<UInt32>(SECTION_OSD, CONF_TOGGLEKEY, 112);
        m_conf.align = ConfigGetStatsRendererAlignment(GetConfigValue(SECTION_OSD, CONF_ALIGN, 1));
        m_conf.offset = GetConfigValue(SECTION_OSD, CONF_OFFSET, "4 4");
        m_conf.font_scale = GetConfigValue(SECTION_OSD, CONF_SCALE, "1 0.9");
        m_conf.font_autoscale = GetConfigValueBool(SECTION_OSD, CONF_AUTOSCALE, true);
        m_conf.scale_to_window = GetConfigValueBool(SECTION_OSD, CONF_SCALETOWINDOW, true);
        m_conf.font = GetConfigValue<Font>(SECTION_OSD, CONF_FONT, Font::DroidSans);
        m_conf.font_file = GetConfigValue(SECTION_OSD, CONF_FONTFILE, "");
        m_conf.font_color = GetConfigValue(SECTION_OSD, CONF_FONTCOLOR, "21 255 18 240");
        m_conf.font_outline_color = GetConfigValue(SECTION_OSD, CONF_FONTOUTLINECOLOR, "0 0 0");
        m_conf.outline_size = GetConfigValueFloat(SECTION_OSD, CONF_FONTOUTLINEOFFSET, 1.0f);
    }
    ...
    void DOSD::PostLoadConfig()
    {
        ...
        if (!rd->IsOK()) {
            _MESSAGE("[OSD] Render driver unavailable, disabling");
        ...
        if (m_conf.enabled)
        {
            if (!m_conf.key || m_conf.key >= InputMap::kMaxMacros) {
                _MESSAGE("[OSD] Invalid toggle key, resetting to default");
    ...
    {
        auto handle = IF4SE::GetSingleton().ModuleHandle();
    ...
    const wchar_t* DOSD::StatsRendererCallback_LoadTime()
    {
        if (loading) {
            m_stats.end = clock();
            if (m_conf.loading_time) {
                ::_snwprintf_s(m_Instance.m_bufStats5, _TRUNCATE, L"\nLoading time: %.2f sec", ((double)m_stats.end - m_stats.start) / ((double)CLOCKS_PER_SEC));
            }
            if (m_conf.bare_loading_time) {
        ...
        else if (last)
        {
            ...
            m_stats.timeout = clock();
            if (m_conf.delay <= ((double)m_stats.timeout - m_stats.end) / ((double)CLOCKS_PER_SEC)) {
                ::_snwprintf_s(m_Instance.m_bufStats5, _TRUNCATE, L"");
                last = false;
            }
        }

        return m_Instance.m_bufStats5;
    }
    ...
    void DOSD::Initialize(D3D11CreateEventPost* a_data)
    {
        try
        {
            ...
            if (!m_conf.font_file.empty())
            {
                ...
                _MESSAGE("[OSD] Loading OSD font from '%s'", StrHelpers::ToNative(file).c_str());

                if (!(res = renderer->Load(file.c_str()))) {
                    _MESSAGE("[OSD] Couldn't load font, falling back to built-in");
                }
                else {
                    isCustom = true;
                }
            ...
            if (res)
            {
                ...
                if (m_conf.vram || m_conf.all)
                {
                    if (SUCCEEDED(a_data->m_pAdapter->QueryInterface(IID_PPV_ARGS(m_adapter.GetAddressOf())))) {
                        ...
                        _MESSAGE("[OSD] Failed to get IDXGIAdapter3, DXGI 1.4 or later is required to display VRAM usage info");
                    }
                }

                if (m_conf.loading_time || m_conf.bare_loading_time)
                {
                    renderer->AddCallback(StatsRendererCallback_LoadTime);
                }

                m_statsRenderer = std::move(renderer);
            }
            else {
                _MESSAGE("[OSD] Couldn't load OSD renderer");
            }
        }
        catch (const std::exception& e)
        {
            _MESSAGE("[OSD] Exception thrown while initializing stats renderer: %s", e.what());
        }
    }
