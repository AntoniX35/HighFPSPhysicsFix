...
namespace SDT
{
    ...
    bool IHook::InstallHooks()
    {
        ...
        for (const auto& hdesc : m_hooks)
        {
            ...
            {
                _MESSAGE("[SkipINI] BranchTrampoline::Write5Call %llX -> %llX", hdesc.wc_target, hdesc.wc_hook);
                IF4SE::GetBranchTrampoline().Write5Call(hdesc.wc_target, hdesc.wc_hook);
                c++;
            }
            else if (hdesc.type == HookDescriptor::HookType::kWR6Call)
            {
                _MESSAGE("[SkipINI] BranchTrampoline::Write6Call %llX -> %llX", hdesc.wc_target, hdesc.wc_hook);
                IF4SE::GetBranchTrampoline().Write6Call(hdesc.wc_target, hdesc.wc_hook);
                c++;
            }
        }

        _MESSAGE("[SkipINI] %u hook(s) installed", c);
